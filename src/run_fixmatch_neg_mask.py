...
from trainer.trainer_fixmatch_neg_mask import trainer_fixmatch
...
def run(config):
    ...
    file_mask = open("mask_0p95.txt", "w") 
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        train_losses = trainer_fixmatch(model, teacher, train_loader, optimizer, criterion, c_w, alpha, epoch, file_mask)
        print('#### Training ####')
...
if __name__ == "__main__":
    ...
    seeds = [7]#,81,91,101,111,122]
    baseline_type = [0]
    config = {
        'logdir': '../logs',
        'exp_name': 'fixmatch_mask_0p95',
        'mode': 0,
        'coarse': 0,
        'data_path': '../data',
        'hparams': {
