...
from model.Attention import DecisionLevelSingleAttention, DecisionLevelSingleAttention_bias
...
def create_model_bias(params, no_grad=False):
    if 'n_features' not in params:
        n_features = 128
    else:
        n_features = params['n_features']
    model = DecisionLevelSingleAttention_bias(n_features,
                                        params['n_classes'],
                                        params['n_layers'],
                                        n_features,
                                        params['drop_rate'])
