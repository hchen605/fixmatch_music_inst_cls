...
class Attention_bias(nn.Module):
    ...
    def forward(self, x):
        ...
        #bias = bias / torch.sum(bias, dim = 2)
...
class DecisionLevelSingleAttention_bias(nn.Module):
    def __init__(self, freq_bins, classes_num, emb_layers, hidden_units, drop_rate, batch_norm=True):

        super(DecisionLevelSingleAttention_bias, self).__init__()
